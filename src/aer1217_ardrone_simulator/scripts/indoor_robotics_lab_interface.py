...
class ROSLabInterface(object):
    ...
    def __init__(self):
        ...
        self.pub_estimated_state_rviz = rospy.Publisher('/aer1217_ardrone/estimated_state_rviz', PoseStamped, queue_size=30)
    ...
    def update_motor_speeds(self, event):
        ...
        if(dt<(0.1/ self.onboard_loop_frequency)):
            return#don't do any thing if the simulation is paused
    ...
    def update_quadrotor_state(self, gazebo_state_msg):
        ...
        pose_stamped.header.stamp = self.quadrotor_state.header.stamp
        pose_stamped.header.frame_id = "/map"
        pose_stamped.pose.position.x = self.quadrotor_state.transform.translation.x
        pose_stamped.pose.position.y = self.quadrotor_state.transform.translation.y
        pose_stamped.pose.position.z = self.quadrotor_state.transform.translation.z

        pose_stamped.pose.orientation.x = self.quadrotor_state.transform.rotation.x
        pose_stamped.pose.orientation.y = self.quadrotor_state.transform.rotation.y
        pose_stamped.pose.orientation.z = self.quadrotor_state.transform.rotation.z
        pose_stamped.pose.orientation.w = self.quadrotor_state.transform.rotation.w
        self.pub_estimated_state_rviz.publish(pose_stamped)
        self.seq+=1
