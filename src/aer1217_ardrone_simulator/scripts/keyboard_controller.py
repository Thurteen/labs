...
class KeyMapping(object):
    ...
    #####################################33
    Reset        = QtCore.Qt.Key.Key_Space
    ThrottleCut      = QtCore.Qt.Key.Key_K # kill switch
    #########################################3
    Takeoff          = QtCore.Qt.Key.Key_O
    Land             = QtCore.Qt.Key.Key_L
    ###For lab 2:
    LinearTrajectory       = QtCore.Qt.Key.Key_N
    CircularTrajectory     = QtCore.Qt.Key.Key_M
    StartTrajctoryFollow   = QtCore.Qt.Key.Key_T
    ...
    def keyPressEvent(self, event):
        ...
        if controller is not None and not event.isAutoRepeat():
            ...
            if key == KeyMapping.ThrottleCut:
                controller.SendThrottleCut()
            elif key == KeyMapping.Reset:
                controller.SendReset() # modify to prevent damage to quad
            ...
            ###LAB2:
                ...
                controller.SendLinearTrajectory()
            elif key == KeyMapping.CircularTrajectory:
                controller.SendCircularTrajectory()
            elif key == KeyMapping.StartTrajctoryFollow:
                controller.SendStartTrajectoryFollow()
            ##################################################################
            else:
